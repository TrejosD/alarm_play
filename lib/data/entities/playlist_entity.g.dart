...
const PlaylistSchema = CollectionSchema(
  ...
  indexes: {},
  links: {},
  embeddedSchemas: {r'PlayListTrack': PlayListTrackSchema},
...
int _playlistEstimateSize(
  ...
  bytesCount += 3 + object.tracks.length * 3;
  {
    ...
    for (var i = 0; i < object.tracks.length; i++) {
      ...
      bytesCount +=
          PlayListTrackSchema.estimateSize(value, offsets, allOffsets);
    }
  }
  return bytesCount;
}
...
void _playlistSerialize(
  ...
  writer.writeObjectList<PlayListTrack>(
    offsets[2],
    allOffsets,
    PlayListTrackSchema.serialize,
    object.tracks,
  );
...
extension PlaylistQueryObject
    on QueryBuilder<Playlist, Playlist, QFilterCondition> {
  QueryBuilder<Playlist, Playlist, QAfterFilterCondition> tracksElement(
      FilterQuery<PlayListTrack> q) {
    return QueryBuilder.apply(this, (query) {
...
extension PlaylistQueryProperty
  ...
  QueryBuilder<Playlist, List<PlayListTrack>, QQueryOperations>
      tracksProperty() {
    return QueryBuilder.apply(this, (query) {
...
const PlayListTrackSchema = Schema(
  ...
  estimateSize: _playListTrackEstimateSize,
  serialize: _playListTrackSerialize,
  deserialize: _playListTrackDeserialize,
  deserializeProp: _playListTrackDeserializeProp,
);

int _playListTrackEstimateSize(
...
P _playListTrackDeserializeProp<P>(
  IsarReader reader,
  int propertyId,
  int offset,
  Map<Type, List<int>> allOffsets,
...
extension PlayListTrackQueryObject
    on QueryBuilder<PlayListTrack, PlayListTrack, QFilterCondition> {}
